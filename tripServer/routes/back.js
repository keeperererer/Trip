...
router.post('/updateUserInfo',function(req,res,next) {
  ...
  // res.json(name)
  pool.query(`UPDATE user_info SET name="${name}",userName="${userName}" WHERE userId="${userId}"`,function(err,results,fields){
    if(err) {
      data.code = 500
      data.msg = err
    };
...
router.post('/deleteUserInfo',function(req,res,next){
  ...
  pool.query(`DELETE FROM user_info WHERE userId="${userId}"`,function(err,results,fields){
    if(err){
      data.code = 500
      data.msg = err
    };
...
router.post('/addUserInfo',function(req,res,next) {
  let name = req.body.name,
      ...
      userId = req.body.userId;
  // res.json(name)
  pool.query(`INSERT INTO user_info SET name="${name}",userId="${userId}",userName="${userName}"`,function(err,results,fields){
    if(err) {
      data.code = 500
      data.msg = err
    };
...
router.post('/updateTotalData',function(req,res,next) {
  ...
  // res.json(name)
  pool.query(`UPDATE total_data SET allWalk="${allWalk}",allRun="${allRun}",allCycle="${allCycle}",allDrive="${allDrive}",allBus="${allBus}",allTaxi="${allTaxi}" WHERE userId="${userId}"`,function(err,results,fields){
    if(err) {
      data.code = 500
      data.msg = err
    };
...
router.post('/deleteTotalData',function(req,res,next){
  let userId = req.body.userId;
  pool.query(`DELETE FROM total_data WHERE userId="${userId}"`,function(err,results,fields){
    if(err){
      data.code = 500
      data.msg = err
    };
...
router.post('/addTotalData',function(req,res,next) {
  let userId = req.body.userId,
      ...
      allTaxi = req.body.allTaxi;
  // res.json(name)
  pool.query(`INSERT INTO total_data SET userId="${userId}",allWalk="${allWalk}",allRun="${allRun}",allCycle="${allCycle}",allDrive="${allDrive}",allBus="${allBus}",allTaxi="${allTaxi}"`,function(err,results,fields){
    if(err) {
      data.code = 500
      data.msg = err
    };
...
router.post('/updateUser',function(req,res,next) {
  let passWord = req.body.passWord,
      ...
      userId = req.body.userId;
  // res.json(name)
  pool.query(`UPDATE user SET userName="${userName}",passWord="${passWord}" WHERE userId="${userId}"`,function(err,results,fields){
    if(err) {
      data.code = 500
      data.msg = err
    };
...
router.post('/deleteUser',function(req,res,next){
  let userId = req.body.userId;
  pool.query(`DELETE FROM user WHERE userId="${userId}"`,function(err,results,fields){
    if(err){
      data.code = 500
      data.msg = err
    };
...
router.post('/addUser',function(req,res,next) {
  let passWord = req.body.passWord,
      ...
      userId = req.body.userId;
  // res.json(name)
  pool.query(`INSERT INTO user SET userId="${userId}",userName="${userName}",passWord="${passWord}"`,function(err,results,fields){
    if(err) {
      data.code = 500
      data.msg = err
    };
