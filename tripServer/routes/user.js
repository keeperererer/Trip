    ...
    if (results && results.length) {
      ...
      pool.query(
        ...
        function (err, results, fields) {
          ...
          console.log('注册', results.insertId)
          if (results && results.affectedRows) {
            let totalId = results.insertId
            pool.query(
              `INSERT INTO total_data (userId,allWalk,allRun,allCycle,allDrive,allBus,allTaxi) VALUES (${totalId},${allDistanceData.allWalk},${allDistanceData.allRun},${allDistanceData.allCycle},${allDistanceData.allDrive},${allDistanceData.allBus},${allDistanceData.allTaxi})`,
              function (err, results) {
                if (err) {
                  data.msg = err
                }
                console.log('total_data', results)
              }
            )
            pool.query(
